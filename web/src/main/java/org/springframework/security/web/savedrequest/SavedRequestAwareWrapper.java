 ...
 * <p>
 * Note that not all request parameters in the original request are emulated by this wrapper. Nevertheless, the
 * important data from the original request is emulated and this should prove adequate for most purposes (in particular
...
public class SavedRequestAwareWrapper extends HttpServletRequestWrapper {
    ...
    /**
     * If the parameter is available from the wrapped request then the request has been forwarded/included to a URL with
     * parameters, either supplementing or overriding the saved request values.
     * <p>
     * In this case, the value from the wrapped request should be used.
     * <p>
     * If the value from the wrapped request is null, an attempt will be made to retrieve the parameter from the saved
     * request.
     */
    @Override
