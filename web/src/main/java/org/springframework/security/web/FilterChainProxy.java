 ...
 * , in that the remainder of the original or {@code FilterChainProxy}-declared filter chain will not be called.
 * <h3>Request Firewalling</h3> An {@link HttpFirewall} instance is used to validate incoming requests and create a
 * wrapped request which provides consistent path values for matching against. See {@link DefaultHttpFirewall}, for more
 * information on the type of attacks which the default implementation protects against. A custom implementation can be
 * injected to provide stricter control over the request contents or if an application needs to support certain types of
 * request which are rejected by default.
 * <p>
