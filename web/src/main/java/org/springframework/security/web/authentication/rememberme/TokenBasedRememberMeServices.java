 ...
 * <p>
 * This implementation does not rely on an external database, so is attractive for simple applications. The cookie will
 * be valid for a specific period from the date of the last
 * {@link #loginSuccess(HttpServletRequest, HttpServletResponse, Authentication)}. As per the interface contract, this
 * method will only be called when the principal completes a successful interactive authentication. As such the time
 * period commences from the last authentication attempt where they furnished credentials - not the time period they
 * last logged in via remember-me. The implementation will only send a remember-me token if the parameter defined by
 ...
 * {@link org.springframework.security.core.userdetails.UserDetails}. This is also necessary so that the user's password
 * is available and can be checked as part of the encoded cookie.
 * <p>
 ...
 * approaches to recovering from a remember-me token being left on a public machine (e.g. kiosk system, Internet cafe
 ...
 * database for remember-me services). High security applications should be aware of this occasionally undesired
 ...
 * database-based implementation if you require a more secure remember-me approach (see
 ...
 * 14 days, but the negative value will be used for the <tt>maxAge</tt> property of the cookie, meaning that it will not
