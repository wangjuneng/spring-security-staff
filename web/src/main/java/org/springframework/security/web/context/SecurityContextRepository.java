 ...
 * <p>
 * Used by {@link SecurityContextPersistenceFilter} to obtain the context which should be used for the current thread of
 * execution and to store the context once it has been removed from thread-local storage and the request has completed.
 * <p>
 * The persistence mechanism used will depend on the implementation, but most commonly the <tt>HttpSession</tt> will be
 * used to store the context.
 *
 * @author Luke Taylor
 * @since 3.0
...
public interface SecurityContextRepository {
     ...
     * the request or response (or both), allowing them to access implementation-specific state for the request. The
     * values obtained from the holder will be passed on to the filter chain and also to the <tt>saveContext</tt> method
     * when it is finally called. Implementations may wish to return a subclass of
