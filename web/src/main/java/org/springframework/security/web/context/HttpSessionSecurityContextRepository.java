 ...
 * between requests.
 * <p>
 * The {@code HttpSession} will be queried to retrieve the {@code SecurityContext} in the <tt>loadContext</tt> method
 * (using the key {@link #SPRING_SECURITY_CONTEXT_KEY} by default). If a valid {@code SecurityContext} cannot be
 * obtained from the {@code HttpSession} for whatever reason, a fresh {@code SecurityContext} will be created by calling
 * by {@link SecurityContextHolder#createEmptyContext()} and this instance will be returned instead.
 * <p>
 * When <tt>saveContext</tt> is called, the context will be stored under the same key, provided
 * <ol>
 * <li>The value has changed</li>
 * <li>The configured <tt>AuthenticationTrustResolver</tt> does not report that the contents represent an anonymous user
 * </li>
 * </ol>
 * <p>
 * With the standard configuration, no {@code HttpSession} will be created during <tt>loadContext</tt> if one does not
 * already exist. When <tt>saveContext</tt> is called at the end of the web request, and no session exists, a new
 * {@code HttpSession} will <b>only</b> be created if the supplied {@code SecurityContext} is not equal to an empty
 * {@code SecurityContext} instance. This avoids needless <code>HttpSession</code> creation, but automates the storage
 * of changes made to the context during the request. Note that if {@link SecurityContextPersistenceFilter} is
 * configured to eagerly create sessions, then the session-minimisation logic applied here will not make any difference.
 * If you are using eager session creation, then you should ensure that the <tt>allowSessionCreation</tt> property of
 * this class is set to <tt>true</tt> (the default).
 * <p>
 * If for whatever reason no {@code HttpSession} should <b>ever</b> be created (for example, if Basic authentication is
 * being used or similar clients that will never present the same {@code jsessionid}), then
 * {@link #setAllowSessionCreation(boolean) allowSessionCreation} should be set to <code>false</code>. Only do this if
 * you really need to conserve server memory and ensure all classes using the {@code SecurityContextHolder} are designed
 * to have no persistence of the {@code SecurityContext} between web requests.
 *
 * @author Luke Taylor
 * @since 3.0
 */
public class HttpSessionSecurityContextRepository implements SecurityContextRepository {
     ...
     * <p>
     * If the session is null, the context object is null or the context object stored in the session is not an instance
     * of {@code SecurityContext}, a new context object will be generated and returned.
     */
    ...
    private SecurityContext readSecurityContextFromSession(HttpSession httpSession) {
        ...
        // we no have the security context object from the session
     ...
     *
     * @param disableUrlRewriting set to <tt>true</tt> to disable URL encoding methods in the response wrapper and
     *            prevent the use of <tt>jsessionid</tt> parameters.
     */
    ...
    final class SaveToSessionResponseWrapper extends SaveContextOnUpdateOrErrorResponseWrapper {
         ...
         * @param httpSessionExistedAtStartOfRequest indicates whether there was a session in place before the filter
         *            chain executed. If this is true, and the session is found to be null, this indicates that it was
         *            invalidated during the request and a new session will now be created.
         * @param context the context before the filter chain executed. The context will only be stored if it or its
         *            contents changed during the request.
         */
        ...
        private HttpSession createNewSessionIfAllowed(SecurityContext context) {
            ...
            if (contextObject.equals(context)) {
                if (logger.isDebugEnabled()) {
                    logger
                        .debug("HttpSession is null, but SecurityContext has not changed from default empty context: ' "
    ...
    /**
     * Sets the {@link AuthenticationTrustResolver} to be used. The default is {@link AuthenticationTrustResolverImpl}.
     *
     * @param trustResolver the {@link AuthenticationTrustResolver} to use. Cannot be null.
     */
