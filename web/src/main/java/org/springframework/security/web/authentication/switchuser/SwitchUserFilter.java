 ...
 * <p>
 * This filter is similar to Unix 'su' however for Spring Security-managed web applications. A common use-case for this
 * feature is the ability to allow higher-authority users (e.g. ROLE_ADMIN) to switch to a regular user (e.g.
 * ROLE_USER).
 * <p>
 * This filter assumes that the user performing the switch will be required to be logged in as normal (i.e. as a
 * ROLE_ADMIN user). The user will then access a page/controller that enables the administrator to specify who they wish
 * to become (see <code>switchUserUrl</code>).
 * <p>
 ...
 * <p>
 * On a successful switch, the user's <code>SecurityContext</code> will be updated to reflect the specified user and
 * will also contain an additional
 * {@link org.springframework.security.web.authentication.switchuser.SwitchUserGrantedAuthority} which contains the
 * original user. Before switching, a check will be made on whether the user is already currently switched, and any
 * current switch will be exited to prevent "nested" switches.
 * <p>
 * To 'exit' from a user context, the user needs to access a URL (see <code>exitUserUrl</code>) that will switch back to
 * the original user as identified by the <code>ROLE_PREVIOUS_ADMINISTRATOR</code>.
 * <p>
 * To configure the Switch User Processing Filter, create a bean definition for the Switch User processing filter and
 * add to the filterChainProxy. Note that the filter must come <b>after</b> the <tt>FilterSecurityInteceptor</tt> in the
 * chain, in order to apply the correct constraints to the <tt>switchUserUrl</tt>. Example:
 *
 * <pre>
...
public class SwitchUserFilter extends GenericFilterBean implements ApplicationEventPublisherAware, MessageSourceAware {
    ...
    private Authentication attemptExitUser(HttpServletRequest request) {
     ...
     *
     * @param userDetailsService The <tt>UserDetailService</tt> which will be used to load information for the user that
     ...
     * {@link #setSuccessHandler(AuthenticationSuccessHandler) setSuccessHandler} instead if you need more customized
     * behaviour.
     *
    ...
    /**
     * Sets the URL to which a user should be redirected if the switch fails. For example, this might happen because the
     * account they are attempting to switch to is invalid (the user doesn't exist, account is locked etc).
     * <p>
     * If not set, an error message will be written to the response.
     * <p>
     * Use {@link #setFailureHandler(AuthenticationFailureHandler) failureHandler} instead if you need more customized
     * behaviour.
     *
    ...
    /**
     * Find the original <code>Authentication</code> object from the current user's granted authorities. A successfully
     * switched user should have a <code>SwitchUserGrantedAuthority</code> that contains the original source user
