 ...
 * <p>
 * The current <code>Authentication</code> will be inspected to determine if the principal has a particular level of
 * authentication. The "FULLY" authenticated option means the user is authenticated fully (i.e.
 * {@link org.springframework.security.authentication.AuthenticationTrustResolver#isAnonymous(Authentication)} is false
 * and {@link org.springframework.security.authentication.AuthenticationTrustResolver#isRememberMe(Authentication)} is
 * false). The "REMEMBERED" will grant access if the principal was either authenticated via remember-me OR is fully
 * authenticated. The "ANONYMOUSLY" will grant access if the principal was authenticated via remember-me, OR
