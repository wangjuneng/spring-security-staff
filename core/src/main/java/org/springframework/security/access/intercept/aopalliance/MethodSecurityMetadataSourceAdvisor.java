...
/**
 * Advisor driven by a {@link MethodSecurityMetadataSource}, used to exclude a {@link MethodInterceptor} from public
 * (non-secure) methods.
 * <p>
 * Because the AOP framework caches advice calculations, this is normally faster than just letting the
 * <code>MethodInterceptor</code> run and find out itself that it has no work to do.
 * <p>
 * This class also allows the use of Spring's {@code DefaultAdvisorAutoProxyCreator}, which makes configuration easier
 * than setup a <code>ProxyFactoryBean</code> for each object requiring security. Note that autoproxying is not
 * supported for BeanFactory implementations, as post-processing is automatic only for application contexts.
 * <p>
...
public class MethodSecurityMetadataSourceAdvisor extends AbstractPointcutAdvisor implements BeanFactoryAware {
     ...
     * bean name should be set. This prevents eager instantiation of the interceptor (and hence the
     * AuthenticationManager). See SEC-773, for example. The metadataSourceBeanName is used rather than a direct
