 ...
 * <p>
 * <tt>AuthenticationProvider</tt>s are usually tried in order until one provides a non-null response. A non-null
 * response indicates the provider had authority to decide on the authentication request and no further providers are
 * tried. If a subsequent provider successfully authenticates the request, the earlier authentication exception is
 ...
 * <code>ProviderManager</code> will throw a <code>ProviderNotFoundException</code>. A parent
 * {@code AuthenticationManager} can also be set, and this will also be tried if none of the configured providers can
 ...
 * {@code Authentication} object, if it implements the {@link CredentialsContainer} interface. This behaviour can be
 * controlled by modifying the {@link #setEraseCredentialsAfterAuthentication(boolean)
 ...
 * {@link DefaultAuthenticationEventPublisher} which maps common exceptions to events (in the case of authentication
 * failure) and publishes an {@link org.springframework.security.authentication.event.AuthenticationSuccessEvent
...
public class ProviderManager implements AuthenticationManager, MessageSourceAware, InitializingBean {
     ...
     * <p>
     * If more than one <code>AuthenticationProvider</code> supports the passed <code>Authentication</code> object, the
     * first one able to successfully authenticate the <code>Authentication</code> object determines the
     * <code>result</code>, overriding any possible <code>AuthenticationException</code> thrown by earlier supporting
     * <code>AuthenticationProvider</code>s. On successful authentication, no subsequent
     * <code>AuthenticationProvider</code>s will be tried. If authentication was not successful by any supporting
     * <code>AuthenticationProvider</code> the last thrown <code>AuthenticationException</code> will be rethrown.
     *
     ...
     *
     * @param eraseSecretData set to {@literal false} to retain the credentials data in memory. Defaults to
     *            {@literal true}.
     */
