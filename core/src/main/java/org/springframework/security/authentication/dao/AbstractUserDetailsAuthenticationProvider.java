...
public abstract class AbstractUserDetailsAuthenticationProvider implements AuthenticationProvider, InitializingBean,
     ...
     * authentication request. Generally a subclass will at least compare the {@link Authentication#getCredentials()}
     * with a {@link UserDetails#getPassword()}. If custom logic is needed to compare additional properties of
     * <code>UserDetails</code> and/or <code>UsernamePasswordAuthenticationToken</code>, these should also appear in
     * this method.
     *
    ...
    protected Authentication createSuccessAuthentication(Object principal, Authentication authentication,
        ...
        UsernamePasswordAuthenticationToken result = new UsernamePasswordAuthenticationToken(principal,
            user.getAuthorities(), authoritiesMapper.mapAuthorities(user.getAuthorities()));
    ...
    /**
     * Allows subclasses to actually retrieve the <code>UserDetails</code> from an implementation-specific location,
     * with the option of throwing an <code>AuthenticationException</code> immediately if the presented credentials are
     * incorrect (this is especially useful if it is necessary to bind to a resource as the user in order to obtain or
     * generate a <code>UserDetails</code>).
     * <p>
     * Subclasses are not required to perform any caching, as the <code>AbstractUserDetailsAuthenticationProvider</code>
     * will by default cache the <code>UserDetails</code>. The caching of <code>UserDetails</code> does present
     * additional complexity as this means subsequent requests that rely on the cache will need to still have their
     * credentials validated, even if the correctness of credentials was assured by subclasses adopting a binding-based
     * strategy in this method. Accordingly it is important that subclasses either disable caching (if they want to
     * ensure that this method is the only method that is capable of authenticating a request, as no
     * <code>UserDetails</code> will ever be cached) or ensure subclasses implement
     * {@link #additionalAuthenticationChecks(UserDetails, UsernamePasswordAuthenticationToken)} to compare the
     * credentials of a cached <code>UserDetails</code> with subsequent authentication requests.
     * </p>
     * <p>
     * Most of the time subclasses will not perform credentials inspection in this method, instead performing it in
     * {@link #additionalAuthenticationChecks(UserDetails, UsernamePasswordAuthenticationToken)} so that code related to
    ...
    /**
     * Sets the policy will be used to verify the status of the loaded <tt>UserDetails</tt> <em>before</em> validation
     * of the credentials takes place.
     *
