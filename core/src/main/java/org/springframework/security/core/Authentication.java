...
public interface Authentication extends Principal, Serializable {
     ...
     * <p>
     * Implementations should ensure that modifications to the returned collection array do not affect the state of the
     * Authentication object, or use an unmodifiable instance.
     * </p>
     *
    ...
    /**
     * Stores additional details about the authentication request. These might be an IP address, certificate serial
     * number etc.
     *
     ...
     * request.
     * <p>
     * The <tt>AuthenticationManager</tt> implementation will often return an <tt>Authentication</tt> containing richer
     * information as the principal for use by the application. Many of the authentication providers will create a
     ...
     * <code>AuthenticationManager</code>. Typically an <code>AuthenticationManager</code> (or, more often, one of its
     * <code>AuthenticationProvider</code>s) will return an immutable authentication token after successful
     * authentication, in which case that token can safely return <code>true</code> to this method. Returning
     * <code>true</code> will improve performance, as calling the <code>AuthenticationManager</code> for every request
     ...
     * <p>
     * Implementations should <b>always</b> allow this method to be called with a <code>false</code> parameter, as this
     * is used by various classes to specify the authentication token should not be trusted. If an implementation wishes
     * to reject an invocation with a <code>true</code> parameter (which would indicate the authentication token is
     * trusted - a potential security risk) the implementation should throw an {@link IllegalArgumentException}.
     *
