...
/**
 * Allows finding parameter names using the value attribute of any number of {@link Annotation} instances. This is
 * useful when needing to discover the parameter names of interfaces with Spring Security's method level security. For
 ...
 * </pre>
 * <p>
 * It is common for users to use {@link AnnotationParameterNameDiscoverer} in conjuction with
 * {@link PrioritizedParameterNameDiscoverer}. In fact, Spring Security's {@link DefaultSecurityParameterNameDiscoverer}
 * (which is used by default with method level security) extends {@link PrioritizedParameterNameDiscoverer} and will
 * automatically support both {@link P} and Spring Data's Param annotation if it is found on the classpath.
 * </p>
 * <p>
 * It is important to note that if a single parameter name has a supported annotation on it then all methods are
 * resolved using {@link AnnotationParameterNameDiscoverer}. For example, consider the following:
 * </p>
 *
 * <pre>
 * import org.springframework.security.access.method.P;
 ...
 * </pre>
 * <p>
 * The result of finding parameters on the previous sample will be <code>new String[] { "to", null}</code> since only a
 * single parameter contains an annotation. This is mostly due to the fact that the fallbacks for
...
public class AnnotationParameterNameDiscoverer implements ParameterNameDiscoverer {
    ...
    /**
     * Finds the parameter name from the provided {@link Annotation}s or null if it could not find it. The search is
     * done by looking at the value property of the {@link #annotationClassesToUse}.
     *
