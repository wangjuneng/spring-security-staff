...
/**
 * Basic implementation of {@link TokenService} that is compatible with clusters and across machine restarts, without
 ...
 * </p>
 * <p>
 * In the above, <code>creationTime</code>, <code>tokenKey</code> and <code>extendedInformation</code> are equal to that
 * stored in {@link Token}. The <code>Sha512Hex</code> includes the same payload, plus a <code>serverSecret</code>.
 * </p>
 * <p>
 * The <code>serverSecret</code> varies every millisecond. It relies on two static server-side secrets. The first is a
 * password, and the second is a server integer. Both of these must remain the same for any issued keys to subsequently
 * be recognised. The applicable <code>serverSecret</code> in any millisecond is computed by <code>password</code> + ":"
 * + (<code>creationTime</code> % <code>serverInteger</code>). This approach further obfuscates the actual server secret
 * and renders attempts to compute the server secret more limited in usefulness (as any false tokens would be forced to
 * have a <code>creationTime</code> equal to the computed hash). Recall that framework features depending on token
 * services should reject tokens that are relatively old in any event.
 * </p>
 * <p>
 * A further consideration of this class is the requirement for cryptographically strong pseudo-random numbers. To this
 * end, the use of {@link SecureRandomFactoryBean} is recommended to inject the property.
 * </p>
 * <p>
 ...
 * @author Ben Alex
 */
public class KeyBasedPersistenceTokenService implements TokenService {
