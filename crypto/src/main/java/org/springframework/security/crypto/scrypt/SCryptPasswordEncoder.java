...
/**
 * <p>
 * Implementation of PasswordEncoder that uses the SCrypt hashing function. Clients can optionally supply a cpu cost
 * parameter, a memory cost parameter and a parallelization parameter.
 * </p>
 * <p>
 * A few <a href= "http://bouncy-castle.1462172.n4.nabble.com/Java-Bouncy-Castle-scrypt-implementation-td4656832.html">
 * warnings</a>:
 * </p>
 * <ul>
 * <li>The currently implementation uses Bouncy castle which does not exploit parallelism/optimizations that password
 * crackers will, so there is an unnecessary asymmetry between attacker and defender.</li>
 * <li>Scrypt is based on Salsa20 which performs poorly in Java (on par with AES) but performs awesome (~4-5x faster) on
 * SIMD capable platforms</li>
 * <li>While there are some that would disagree, consider reading - <a
 * href="http://blog.ircmaxell.com/2014/03/why-i-dont-recommend-scrypt.html"> Why I Don't Recommend Scrypt</a> (for
 * password storage)</li>
 * </ul>
 *
...
public class SCryptPasswordEncoder implements PasswordEncoder {
     ...
     *
     * @param cpuCost cpu cost of the algorithm (as defined in scrypt this is N). must be power of 2 greater than 1.
     ...
     * @param parallelization the parallelization of the algorithm (as defined in scrypt this is p) Default is currently
     *            1. Note that the implementation does not currently take advantage of parallelization.
     * @param keyLength key length for the algorithm (as defined in scrypt this is dkLen). The default is currently 32.
     * @param saltLength salt length (as defined in scrypt this is the length of S). The default is currently 64.
     */
    ...
    private String digest(CharSequence rawPassword, byte[] salt) {
        ...
        String params = Long.toString(((int) (Math.log(cpuCost) / Math.log(2)) << 16L) | memoryCost << 8
            | parallelization, 16);
